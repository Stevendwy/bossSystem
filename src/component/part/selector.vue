<template>
  <div class="container">
    <div class="pr-selector"
      ...
      <div class="prs-content">
        <div :class="item.status ? 'prsc-row' : 'prscr-warning'"
          ...
          <div class="prsc-item">
            {{item.label}}
            <!-- {{splitPid(item.label, item.s_pid)[0]}}
            <span v-show="splitPid(item.label, item.s_pid)[0] !== item.label" class="prsc-item-special">
              {{item.s_pid}}
            </span>
            {{splitPid(item.label, item.s_pid)[1]}} -->
          </div>
        </div>
        <div class="load-more"
          v-if="hasNext === 1"
    ...
    <div class="no-list"
      ...
      <div class="img" />
      <span class="remind">抱歉, 暂无此零件供应~</span>
    </div>
  </div>
</template>
