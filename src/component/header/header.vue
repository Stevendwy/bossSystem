  ...
  <div class="header-container">
    <div class="header">
      <div class="header-left">
        <div class="header-left-logo" @click="goHomepage" />
        <div
          :class="naviSelectedPath === paths[0] ? 'header-navi-selected' : 'header-navi'"
          @click="click(paths[0])">特卖会</div>
        <div
          :class="naviSelectedPath === paths[1] ? 'header-navi-selected' : 'header-navi'"
          @click="click(paths[1])">零件搜索</div>
      </div>
      <div class="header-right">
        <div style="padding-right: 10px"
          :class="naviSelectedPath === paths[2] ? 'header-navi-selected shopping-cart' : 'header-navi shopping-cart'"
          @click="click(paths[2])">
          <span class="icon-cart" />
          <span v-show="badgeCount" class="badge-icon">{{badgeCount}}</span>
          购物车
        </div>
        <div
          :class="inBuyer ? 'header-navi-selected' : 'header-navi'"
          @click="click(paths[4])">
          <el-dropdown
            @command="handleBuyerCommand">
            <span class="el-dropdown-link">
              买家中心<i class="el-icon-arrow-down el-icon--right"></i>
            </span>
            <span v-show="untouch.order" class="badge-icon">{{untouch.order}}</span>
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item command="buyed">已买到货品</el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
        </div>
        <div
          :class="inseller ? 'header-navi-selected' : 'header-navi'"
          @click="click(paths[5])"
          v-if="this.userInfo.is_seller">
          <el-dropdown
            :show-timeout="0"
            :hide-timeout="0"
            @command="handleSellerCommand">
            <span class="el-dropdown-link">
              卖家中心<i class="el-icon-arrow-down el-icon--right"></i>
            </span>
            <span v-show="untouch.order_seller" class="badge-icon">{{untouch.order_seller}}</span>
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item command="saled"><span>已卖出货品</span></el-dropdown-item>
              <el-dropdown-item command="hub">库存管理</el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
        </div>
        <div
          :class="inperson ? 'header-navi-selected' : 'header-navi'"
          @click="click(paths[7])">
          <el-dropdown
            :show-timeout="0"
            :hide-timeout="0"
            @command="handlePersonCommand">
            <span class="el-dropdown-link">
              个人中心<i class="el-icon-arrow-down el-icon--right"></i>
            </span>
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item command="mine"><span>我的萤火虫</span></el-dropdown-item>
...
export default {
  ...
  computed: {
    ...
    ...Vuex.mapState(["naviSelectedPath", "badgeCount", "untouch"]),
